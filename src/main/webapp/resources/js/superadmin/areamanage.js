...
function listAreaManagementInfo() {
	$.ajax({
		url : "/CampusShop/superadmin/listarea",
		type : "get",
		dataType : 'json',
...
function areaManagementAdd() {
	...
	$.ajax({
		...
		url : '/CampusShop/superadmin/addarea',// 请求的action路径
...
function areaManagementEdit() {
	...
	$.ajax({
		...
		url : '/CampusShop/superadmin/modifyarea',// 请求的action路径
...
function doDel(areaId) {
	$.messager.confirm('删除提示', '你确定永久删除该数据吗?', function(r) {
		if (r) {
			var url = '/CampusShop/superadmin/removearea?areaId=' + areaId;
			changeStatus(url);
		}
	});
}
...
function batch(flag) {
	if ($('#areaManagementTable').datagrid('getSelected')) {
		...
		$.messager.confirm('删除提示', '你确定永久删除下列区域吗?<br/>' + areaNames.join(','),
				function(r) {
					if (r) {
						var url = '/CampusShop/superadmin/removeareas?areaIdListStr='
